...
namespace MSSolutions.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public LoginController(ILogger<LoginController> logger, MSSolutionsDbContext context, IConfiguration config)
        {
            _logger = logger;
            _context = context;
            _config = config;
        ...
        public async Task<IActionResult> Login([FromBody] LoginRequest request)
        {
            ...
            // Simulate user lookup
                var user = await _context.MsUsers.FirstOrDefaultAsync(u => u.Email == request.Username);
                ...
                var address = await _context.MsAddresses.FirstOrDefaultAsync(a => a.UserId == user.Id && !a.IsBusinessAddress);
                var businessAddress = await _context.MsAddresses.FirstOrDefaultAsync(a => a.UserId == user.Id && a.IsBusinessAddress);

                // Load roles
                var roles = await _context.MsUserRoles
                    .Where(r => r.UserId == user.Id)
                    .Select(r => r.RoleId)
                    .ToListAsync();

                // Load role name map
                var roleNameMap = await _context.MsRoles
                    .AsNoTracking()
                ...
                // Load privileges
                var privileges = await _context.MsUserPrivileges
                ...
                var response = new
                {
                    ...
                    data = new
                    {
                        ...
                        business_address = new
                        {
                            ...
                            location = businessAddress?.StateId,
