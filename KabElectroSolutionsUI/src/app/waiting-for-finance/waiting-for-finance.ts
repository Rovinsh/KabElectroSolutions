...
import { ReviewInvoice } from '../review-invoice/review-invoice';
import {Updatepaymentdetails} from '../updatepaymentdetails/updatepaymentdetails'

@Component({
  selector: 'app-waiting-for-finance',
  imports: [FormsModule,CommonModule, RouterModule,MatIconModule],
  templateUrl: './waiting-for-finance.html',
  styleUrl: './waiting-for-finance.css'
})
export class WaitingForFinance {
constructor(private apiService: ApiService,private dialog: MatDialog) {}
  ...
  isLoading = false;
  isReassign = false;
  buttonText = "ACCEPT/REJECT CLAIM";
...
filter = {
  ...
  openAssignPartnerModal(claimId: number) {
    let dialogRef;
    if(this.isReassign){
  dialogRef = this.dialog.open(Updatepaymentdetails, {
    width: '40%',
    maxWidth: '95vw',  
        maxHeight: '90vh',  
        height: 'auto',
...
{
dialogRef = this.dialog.open(ReviewInvoice, {
    width: '40%',
    maxWidth: '95vw',  
        maxHeight: '90vh',  
        height: 'auto',
  ...
  dialogRef!.afterClosed().subscribe(result => {
    if(this.isReassign)
      this.loadClaimsByStatus(this.getSubStatusId("Invoice Accepted By Finance"), false);
    else
    this.loadClaimsByStatus(this.getSubStatusId("Invoice Accepted By Service"), false);
  });
}
...
openClaimDetails(claimId: number): void {
  ...
  ngOnInit(): void {
    ...
    this.loadClaimsByStatus(this.getSubStatusId("Invoice Accepted By Service"), this.isReassign);
  },
      error: (err) => {
...
if(this.isReassign)
{
  this.buttonText = "UPDATE PAYMENT DETAILS";
  this.loadClaimsByStatus(this.getSubStatusId("Invoice Accepted By Finance"), false);
}
else
{
  this.buttonText = "ACCEPT/REJECT CLAIM";
  this.loadClaimsByStatus(this.getSubStatusId("Invoice Accepted By Service"), false);
}
...
loadClaimsByStatus(subStatusId: number, isReassign:boolean): void {
  this.isLoading = true;
  this.apiService.getClaims(`Claims/claims?statusId=${subStatusId}&isReassign=false`).subscribe({
    next: (res) => {
