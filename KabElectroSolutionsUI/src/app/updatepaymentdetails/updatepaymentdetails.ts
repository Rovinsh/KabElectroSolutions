...
@Component({
  selector: 'app-updatepaymentdetails',
  ...
  styleUrl: './updatepaymentdetails.css'
})
export class Updatepaymentdetails {
  ...
  constructor(private dialogRef: MatDialogRef<Updatepaymentdetails>,private apiService: ApiService) {}

  accept() {
    if (!this.remark.trim() || !this.utr.trim()) {
      this.error = true;
      return;
    }
    this.updateClaimStatus(this.claimId,"Call Closed",this.remark, this.utr);    
  }
...
updateClaimStatus(claimId: number,status: string, remarks:string, utr: string) {
    this.isLoading = true;
    this.apiService.AcceptOrRejectClaim(claimId, status, remarks,utr).subscribe({
      next: (res) => {        
